...
# fit
gplam_mgcv <- function(X, U, y, qj=9){
  # model
  b <- gam(y~X+s(U$u1)+s(U$u2)+s(U$u3)-1, family=Gamma(link="log"))
...
# X <- dat$X
# U <- data.frame(u1=dat$u1, u2=dat$u2, u3=dat$u3)
# y <- dat$y1
# b1 <- gplam_mgcv(X, U, y)
# 
# # prep
...
# 
# # plot
# plot(b1$f$f1~b1$u$f1, ylab="f1 fitted", xlab="u1")
# plot(b1$f$f2~b1$u$f2, ylab="f2 fitted", xlab="u2")
# plot(b1$f$f3~b1$u$f3, ylab="f3 fitted", xlab="u3")
# 
# # check beta
# c(2, -1.8)
# b1$beta
...
dat_n1 <- data_gen(n, R)
X <- dat_n1$X
U <- data.frame(u1=dat_n1$u1, u2=dat_n1$u2, u3=dat_n1$u3)
time_n1 <- numeric(R)
list_n1 <- list()
for(j in 1:R){
  name <- paste0("y", j)
  y <- unlist(dat_n1[name])
  time_n1[j] <- system.time(b <- gplam_mgcv(X, U, y), gcFirst=T)[3]
  list_n1[[name]] <- b
...
dat_n2 <- data_gen(n, R)
X <- dat_n2$X
U <- data.frame(u1=dat_n2$u1, u2=dat_n2$u2, u3=dat_n2$u3)
time_n2 <- numeric(R)
list_n2 <- list()
for(j in 1:R){
  name <- paste0("y", j)
  y <- unlist(dat_n2[name])
  time_n2[j] <- system.time(b <- gplam_mgcv(X, U, y), gcFirst=T)[3]
  list_n2[[name]] <- b
  cat("\r", "Model actual:", j, strrep(" ", 20))
}
...
dat_n3 <- data_gen(n, R)
X <- dat_n3$X
U <- data.frame(u1=dat_n3$u1, u2=dat_n3$u2, u3=dat_n3$u3)
time_n3 <- numeric(R)
list_n3 <- list()
for(j in 1:R){
  name <- paste0("y", j)
  y <- unlist(dat_n3[name])
  time_n3[j] <- system.time(b <- gplam_mgcv(X, U, y), gcFirst=T)[3]
  list_n3[[name]] <- b
  cat("\r", "Model actual:", j, strrep(" ", 20))
}

# # save environment
# save.image(file="data/simu_gamma1_mgcv.RData")
# rm(list = ls())

# # load environment
# load("data/simu_gamma1_mgcv.RData")
