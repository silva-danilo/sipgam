...
# # save environment
# save.image(file="simu_1step.RData")
...
# plot phi
par(mar=c(2.6,5.6,1,1), mfrow=c(1,3), cex.lab=2, cex.axis=1.7)
phi_n1 <- sapply(1:R, function(j) sum(list_n1[[name[j]]]$phi)) 
phi_n1 <- phi_n1[!ids_n1]
phi_n2 <- sapply(1:R, function(j) sum(list_n2[[name[j]]]$phi)) 
phi_n2 <- phi_n2[!ids_n2]
phi_n3 <- sapply(1:R, function(j) sum(list_n3[[name[j]]]$phi)) 
